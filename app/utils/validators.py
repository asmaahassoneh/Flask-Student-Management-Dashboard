...
def validate_student_form(name, student_id, course_ids):
...
def validate_user_form(
    username, email, password=None, confirm_password=None, partial=False
):
    username = normalize_text(username)
    email = normalize_email(email)
...
def validate_user_payload(data, partial=False):
    ...
    if "username" in data:
        if not normalize_text(data.get("username")):
